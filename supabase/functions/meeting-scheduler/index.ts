...
/**
 * Retrieve a valid Google access token for a consultant.
 * Looks up stored OAuth credentials in agent_memory, refreshes if needed.
 * Returns { accessToken, eventId } or null if no OAuth setup exists.
 */
async function getGoogleAccessToken(
  consultantId: string,
  cid: string,
): Promise<{ accessToken: string; calendarId: string } | null> {
  // Look up stored OAuth token in agent_memory
  ...
  // Find the entry matching this consultant_id and google_calendar provider
  ...
  // Update stored token in agent_memory
  const updatedContent = {
    ...content,
    access_token: newToken.access_token,
    expires_at: newExpiresAt,
    // Keep the existing refresh_token (Google may return a new one)
  ...
  if (updateError) {
    ...
    // Still return the token — don't fail the request just because we couldn't persist
  }
...
/**
 * Returns an auth header dict for Google API calls.
 * If OAuth is available, returns Bearer token. Otherwise falls back to API key (read-only).
 */
async function getGoogleAuthHeaders(
  ...
  // Try OAuth first (for write operations or if consultant provided)
  if (consultantId) {
...
// ──────────────────────────────────────────────
// Get available time slots from Google Calendar (read-only)
// ──────────────────────────────────────────────
async function getAvailableSlots(
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // Generate multiple time slots per day: 9 AM, 11 AM, 2 PM, 4 PM
...
// ──────────────────────────────────────────────
// Handle: schedule_meeting (existing — find available slots)
// ──────────────────────────────────────────────
...
// ──────────────────────────────────────────────
// Handle: confirm_slot (existing — confirm a slot)
// ──────────────────────────────────────────────
async function handleConfirmSlot(req: Request, cid: string) {
  ...
  try {
    ...
    // Create the calendar event using OAuth
    ...
    if ("error" in authResult) {
      ...
      // Still confirm the meeting in DB, just note the calendar limitation
      return successResponse({
        success: true,
        meeting_id: meeting.id,
...
// ──────────────────────────────────────────────
// Handle: create_meeting — Full CRUD: Create
// Creates a Google Calendar event with attendees, reminders, and Meet link
// ──────────────────────────────────────────────
async function handleCreateMeeting(req: Request, cid: string) {
  ...
  try {
    ...
    // Get OAuth access token — write operation required
    ...
    if (meetingError || !meeting) {
      ...
      // Event was created in Google Calendar — we should note this discrepancy
      return errorResponse(
...
// ──────────────────────────────────────────────
// Handle: update_meeting — Full CRUD: Update
...
// ──────────────────────────────────────────────
// Handle: cancel_meeting — Full CRUD: Delete
// Deletes the Google Calendar event and updates DB status to 'Cancelled'
// ──────────────────────────────────────────────
  ...
  try {
    ...
    if (eventId) {
      ...
      if (calendarResponse.ok || calendarResponse.status === 204) {
        ...
        // Don't fail — we still update the DB status
      }
    } else {
    ...
    // Log lead activity if there's a lead
    if (meeting.lead_id) {
      await supabase.from("lead_activities").insert({
...
// ──────────────────────────────────────────────
// Handle: list_meetings — Full CRUD: List
// Fetches events for a date range from Google Calendar + DB
// ──────────────────────────────────────────────
async function handleListMeetings(req: Request, cid: string) {
  ...
  try {
    // Build query for meetings table
    let query = supabase
      .from("meetings")
      .select(`
        *,
    ...
    // Also try to fetch from Google Calendar if OAuth is available
